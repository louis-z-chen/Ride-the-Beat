...
if(empty($_REQUEST["edit"])) {
  ...
  $sql = "UPDATE playlist_info_view SET
          image_url = '" . $_REQUEST["imageurl"] . "' ,
          name = '" . $_REQUEST["playlistname"] . "' ,
          public = '" . $_REQUEST["public"] . "' ,
          url = '" . $_REQUEST["playlisturl"] . "' ,
          spotify_id = '" . $_REQUEST["playlistid"] . "' ,
          creator_id = $test
          WHERE playlist_id = " . $_REQUEST['edit2'] . "";
}
...
<br />
          ...
          <table class="table table-dark table-hover table-fixed">
            ...
            <tbody>
              <?php while($row = $results->fetch_assoc()) {?>
                ...
                <td class="td-class"><span id="url<?php echo $row["ID"];?>"> <?php echo $row["url"];?></span></td>
                <td class="td-class"><span id="spotify_id<?php echo $row["ID"];?>"> <?php echo $row["spotify_id"];?></span></td>
                <td class="td-class"><span id="image_url<?php echo $row["ID"];?>"> <?php echo $row['image_url'];?></span></td>
                  </span>
                </td>
                <td align="center">
                  <button type="button" class="btn btn-outline-warning edit" value="<?php echo $row['playlist_id']; ?>"><i class="far fa-edit"></i>Edit</button>
                </td>
              </tr>
            <?php } ?>
            </tbody>
      ...
      <!-- Edit Modal HTML -->
    	<div id="edit_playlist" class="modal fade">
    		<div class="modal-dialog modal-dialog-centered">
    			<div class="modal-content">
            <form action="../pages/admin_playlist.php">
              <input type="text" name="edit" value="edit" hidden />
                <input type="text" name="edit2" id="edit2" hidden/>
     					<div class="modal-header">
                ...
                <ul class="form-messages" id="edit_errors"></ul>
    						<div class="form-group">
    							<label>Playlist Image URL</label>
    							<input type="url" name="imageurl" class="form-control" id="efirst">
    						</div>
                <div class="form-group">
    							<label>Playlist Name</label>
    							<input type="text" name="playlistname" class="form-control" id="elast">
    						</div>
    						<div class="form-group">
    							<label>Public</label>
    							<input type="number" min="0" max="1" name="public" class="form-control" id="eemail">
    						</div>
    						<div class="form-group">
    							<label>Playlist URL</label>
    							<input type="url" name="playlisturl" class="form-control" id="eusername">
    						</div>
                <div class="form-group">
    							<label>Spotify Playlist ID</label>
